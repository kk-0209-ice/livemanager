...
チェキ機能を公開アプリ水準へ大幅強化しました。

- 専用「チェキ」タブ
- OpenCV.jsを利用した高精度四隅検出（必要時のみ遅延ロード）
- OpenCVが利用できない場合の内蔵フォールバック
- 手動四隅調整
- 台形・遠近補正
- チェキ実寸比率 54×86 への整形
- 自然補正 / 補正なし / 明るめ
- 90度回転
- 品質スコア（解像度・明るさ・白飛び・黒つぶれ・ぼけ）
- チェキ管理の検索・絞り込み・並べ替え
- メタ情報編集
- 画像をJPEGで端末保存
- ストレージ容量確認・永続保存リクエスト
- 画像込みバックアップ
- 一括スキャンで複数輪郭を高精度検出・台形補正

高精度自動検出にはOpenCV.jsをブラウザで読み込みます。読み込みに失敗した場合も基本検出と手動補正で保存できます。
...
詳細は `V6_CHEKI_SCAN_2.md` を参照してください。
